...
contract("CoinFlipContract", function (accounts) {
  ...
  describe("startGame", function () {
    ...
    it("should fail if ETH & msg.value < gameMinStake", async function () {
      ...
      let wrongEthStake = BET_ETH_MIN.div(new BN("2"));
      await expectRevert(game.startGame(constants.ZERO_ADDRESS, wrongEthStake, creatorHash, CREATOR_REFERRAL, {
        from: CREATOR
      }), "Wrong ETH stake");
    });
    ...
    it("should add game with correct params in games for ETH", async function () {
      ...
      assert.equal(0, game_0.stake.cmp(BET_ETH), "wrong stake for 0");
      ...
      assert.equal(0, game_1.stake.cmp(BET_ETH_1), "wrong stake for 1");
    ...
    it("should add game with correct params in games for Token", async function () {
      ...
      assert.equal(0, game_0.idx.cmp(new BN("0")), "wrong idx for 0");
      assert.equal(0, game_0.stake.cmp(BET_TOKEN), "wrong stake for 0");
      ...
      assert.equal(game_1.creator, CREATOR_1, "wrong creator for 1");
      assert.equal(0, game_1.idx.cmp(new BN("1")), "wrong idx for 1");
      assert.equal(0, game_1.stake.cmp(BET_TOKEN_1), "wrong stake for 1");
      assert.equal(0, game_1.startBlock.cmp(await time.latestBlock()), "wrong startBlock for 1");
      assert.equal(0, game_1.heads.cmp(new BN("0")), "wrong heads for 1");
      assert.equal(0, game_1.tails.cmp(new BN("0")), "wrong tails for 1");
      assert.equal(0, game_1.creatorPrize.cmp(new BN("0")), "wrong creatorPrize for 1");
    ...
    it("should increase playerStakeTotal (_increaseStakes) for ETH", async function () {
      assert.equal(0, (await game.getPlayerStakeTotal.call(constants.ZERO_ADDRESS, {
        from: CREATOR
      ...
      assert.equal(0, (await game.getPlayerStakeTotal.call(testToken.address, {
        from: CREATOR_1
      })).cmp(new BN("0")), "should be 0 before");
    ...
    it("should increase playerStakeTotal for Token", async function () {
      //  approve TestToken
      await testToken.approve(game.address, ether("0.1"), {
        from: CREATOR_1
      });
    ...
    it("should increase betsTotal (_increaseStakes) for ETH", async function () {
    ...
    it("should increase betsTotal (_increaseStakes) for Token", async function () {
      //  approve TestToken
      await testToken.approve(game.address, ether("0.1"), {
        from: CREATOR_1
      });
  ...
  describe("joinGame", function () {
    ...
    it("should fail if Wrong stake for Token", async function () {
    ...
    it("should fail if Wrong stake for ETH", async function () {
      await expectRevert(game.joinGame(constants.ZERO_ADDRESS, 0, 2, OPPONENT_REFERRAL, {
        from: OPPONENT,
        value: ether("1")
      }), "Wrong stake");
    ...
    it("should increase playerStakeTotal (_increaseStakes) for Token", async function () {
      //  0
      assert.equal(0, (await game.getPlayerStakeTotal.call(testToken.address, {
        from: OPPONENT
